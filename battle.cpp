...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case MO_EXTREMITYFIST:
			skillratio += 25 * (7 + sstatus->sp / 4);//zerel 1/10th sp
			break;
	...
	if (sc) {
		...
		if (sc->data[SC_SPIRIT]) {
			if (skill_id == AS_SONICBLOW && sc->data[SC_SPIRIT]->val2 == SL_ASSASIN) {
				ATK_ADDRATE(wd.damage, wd.damage2, map_flag_gvg2(src->m) ? 25 : 25); //+25% dmg on woe/+100% dmg on nonwoe
				RE_ALLATK_ADDRATE(wd, map_flag_gvg2(src->m) ? 25 : 25); //+25% dmg on woe/+100% dmg on nonwoe
				...
				switch(skill_id) {
					...
					case MG_LIGHTNINGBOLT:
						if (sc && sc->data[SC_SPELLFIST] && mflag&BF_SHORT)  {
							skillratio += (sc->data[SC_SPELLFIST]->val4 * 100) + (sc->data[SC_SPELLFIST]->val1 * 100) - 100;// val4 = used bolt level, val2 = used spellfist level. [Rytech]
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch (skill_id) {
		...
		case HW_GRAVITATION:
			...
			break;
		case PA_PRESSURE:
			md.damage = 500 + 900 * skill_lv;
			if (md.damage > 5000) md.damage = 5000;
			break;
		case PA_GOSPEL:
			if (mflag > 0)
		...
		case CR_ACIDDEMONSTRATION:
			...
			if(tstatus->vit+sstatus->int_) //crash fix
				md.damage = (int)((int64)5*tstatus->vit*sstatus->int_*sstatus->int_ / (5*(tstatus->vit+sstatus->int_)));
			else
				md.damage = 0;
			if (tsd) md.damage>>=1;
#endif
...
int64 battle_calc_return_damage(struct block_list* bl, struct block_list *src, int64 *dmg, int flag, uint16 skill_id, bool status_reflect){
	...
	if (flag & BF_SHORT) {//Bounces back part of the damage.
        if (!status_reflect && skill_id == WS_CARTTERMINATION && !status_bl_has_mode(src, MD_STATUS_IMMUNE)) //Ignoring all reflecting (yuyu)
        	{
            		rdamage = 0;
            		return 0;
        	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
		...
		if ( ( ( (TBL_MOB*)target )->mob_id == 1288 && !strcmp( mapindex_id2name(sd->mapindex), "guild_vs1-1" ) ) &&
